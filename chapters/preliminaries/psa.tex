...
\KwIn{LP in standard form (\ref{eq:lp_standard}); Basic and non-basic index-sets $B,N$}
\KwOut{Optimal Solution $(\vec{x}, z)$}
\Loop{
	$\vec{\pi}\transpose \gets \vec{c}_B\transpose \mat{A}_B^{-1}$;
	$\bar{\vec{b}} \gets \mat{A}_B^{-1} \vec{b}$\;
	$\bar{c}_j \gets c_j - \vec{\pi}\transpose \vec{a}_j$;$ \qquad \forall j \in N$\\
	$l \gets \underset{j \in N}{\arg\min} \, \bar{c}_j$;
	$\bar{c}(\vec{\pi}) \gets \bar{c}_l$\;
	\If{$\bar{c}(\vec{\pi}) \geq 0$}{
		\Return{$\left(\left[\bar{\vec{b}}, \vec{0}\right], \vec{c}_B\transpose \vec{x}_B\right)$} \textit{by optimality}
	}
	$\bar{\vec{a}}_l \gets \mat{A}_B^{-1} \vec{a}_l$\;
	...
	$B \gets B \cup \{l\} \subseteq \{s\}$;
	$N \gets N \cup \{s\} \subseteq \{l\}$\;
}
\end{algorithm}