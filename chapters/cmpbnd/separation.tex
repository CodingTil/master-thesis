...
\begin{itemize}
\item	\texttt{ClosedToZHalf} Heuristic: For each $S_j$, we calculate $K_j \coloneqq \sum_{q \in Q(S_j)} \lambda_q^*$. We then select the one where $K_j$ is closest to $\frac{Z}{2}$. Here, $Z$ denotes the number of aggregated subproblems in the current block (see Section \ref{sec:cg_bp_idsp} and Section \ref{sec:cmpbnd_separation_branching}). The intuition behind this is once again to maintain balance within the tree: if $K_j$ was far off from $\frac{Z}{2}$, i.e., either close to $0$ or close to $Z$, branching on $S_j$ would be similar to dichotomous branching, since it would either forbid almost all or almost no solutions (see Section \ref{sec:cg_bp_bp_branching_master}).
